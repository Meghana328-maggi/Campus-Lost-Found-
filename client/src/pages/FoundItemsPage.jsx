    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 p-1 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 w-fit mb-3">
            <Link
              to={`/lost${search ? `?search=${encodeURIComponent(search)}` : ''}`}
            ...
            <span className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-emerald-600 text-white shadow-xs">
              Found Items ({pagination.total || 0})
            </span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Items safely turned in and awaiting verified claims by their legitimate owners.
          </p>
        </div>

        <Link
          to="/report-found"
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow-sm transition self-start md:self-auto"
        >
      ...
      <form onSubmit={handleSearchSubmit} className="relative flex items-center">
        ...
        <input
          ...
          placeholder="Search found belongings by title, brand, storage location, or category..."
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition whitespace-nowrap ${
              category === cat.name
                ...
                : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200/80 dark:border-slate-800 hover:border-emerald-400'
            }`}
          >
            {cat.name}
          </button>
      ...
      <div className="flex items-center justify-between text-xs font-semibold text-slate-500 pt-1">
        <span>Found {pagination.total || 0} discovered item(s)</span>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Try removing applied filters or checking the lost items board."
